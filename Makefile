...
DOCKER_UV = apt-get update && apt-get install -y --no-install-recommends curl ca-certificates && rm -rf /var/lib/apt/lists/* && curl -LsSf https://astral.sh/uv/0.12.17/install.sh | sh && export PATH="$$HOME/.local/bin:$$PATH"
...
.PHONY: generate clean check lint typecheck frontend-check frontend-lint frontend-typecheck frontend-i18n frontend-test backend-check backend-lint backend-lint-all backend-typecheck backend-typecheck-all generate-grpc clean-grpc generate-orval clean-orval
...
frontend-check: frontend-lint frontend-typecheck frontend-i18n frontend-test
