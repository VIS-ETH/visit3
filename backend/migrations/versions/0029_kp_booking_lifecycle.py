...
def _drop_active_booking_indexes() -> None:
    # The index predicates reference the status enum, so they have to be gone
...
def _create_active_booking_indexes(active: str) -> None:
...
def upgrade() -> None:
    ...
    _drop_active_booking_indexes()
    _swap_booking_status_enum(booking_status_with_rejected)
    _create_active_booking_indexes(WITH_REJECTED)
...
def downgrade() -> None:
    ...
    _create_active_booking_indexes(WITHOUT_REJECTED)
